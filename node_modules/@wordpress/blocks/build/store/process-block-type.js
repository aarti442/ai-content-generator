...
function stabilizeSupportConfig(unstableConfig, stableSupportKey) {
  ...
  for (const [key, value] of Object.entries(unstableConfig)) {
    ...
    const stableKey = (_ref = (_EXPERIMENTAL_SUPPORT = _constants.EXPERIMENTAL_SUPPORT_PROPERTIES[stableSupportKey]?.[key]) !== null && _EXPERIMENTAL_SUPPORT !== void 0 ? _EXPERIMENTAL_SUPPORT : _constants.COMMON_EXPERIMENTAL_PROPERTIES[key]) !== null && _ref !== void 0 ? _ref : key;
...
function stabilizeSupports(rawSupports) {
  ...
  for (const [support, config] of Object.entries(rawSupports)) {
    ...
    const stableSupportKey = (_EXPERIMENTAL_SUPPORT2 = _constants.EXPERIMENTAL_SUPPORTS_MAP[support]) !== null && _EXPERIMENTAL_SUPPORT2 !== void 0 ? _EXPERIMENTAL_SUPPORT2 : support;
     ...
     */
    if (support === stableSupportKey && (!(0, _isPlainObject.isPlainObject)(config) || !_constants.EXPERIMENTAL_SUPPORT_PROPERTIES[stableSupportKey] && Object.keys(config).every(key => !_constants.COMMON_EXPERIMENTAL_PROPERTIES[key]))) {
      newSupports[support] = config;
      continue;
    }

    // Stabilize the config value.
    const stableConfig = (0, _isPlainObject.isPlainObject)(config) ? stabilizeSupportConfig(config, stableSupportKey) : config;
    ...
    if (support !== stableSupportKey && Object.hasOwn(rawSupports, stableSupportKey)) {
      ...
      if ((0, _isPlainObject.isPlainObject)(rawSupports[stableSupportKey])) {
        /*
...
const processBlockType = (name, blockSettings) => ({
  ...
  if (settings.deprecated) {
    settings.deprecated = settings.deprecated.map(deprecation => {
      ...
      return Object.fromEntries(Object.entries(filteredDeprecation).filter(([key]) => _constants.DEPRECATED_ENTRY_KEYS.includes(key)));
    });
  }
